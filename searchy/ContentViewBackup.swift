...
//   private var isModelLoaded = false  // New state to track model loading
//
//   func cancelSearch() {
...
//       print("Search triggered with query: \(query) and \(numberOfResults) results.")  // Debug: Search initiation
//
//       // Check if the model is already loaded (skip loading if already loaded)
//       if !isModelLoaded {
//           print("Model not loaded yet, loading the model for the first time...")  // Debug: Model not loaded
//       } else {
//           print("Model already loaded, proceeding with search.")  // Debug: Model loaded
//       }
//
//       cancelSearch()
//
...
//           print("Search started, isSearching set to true.")  // Debug: Search status
//       }
//
...
//       print("Performing search with query: \(query), searching for top \(numberOfResults) results.")  // Debug: Start of search
//
//       guard let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first else {
//           updateState(error: "Failed to access Application Support directory")
//           print("Error: Failed to access Application Support directory.")  // Debug: Error
//           return
//       }
//
//       let appDir = appSupport.appendingPathComponent("searchy")
//       print("Using Application Support directory at path: \(appDir.path)")  // Debug: Application directory path
//
//       do {
//           try FileManager.default.createDirectory(at: appDir, withIntermediateDirectories: true, attributes: nil)
//           print("Created Application Support directory at path: \(appDir.path)")  // Debug: Directory creation
//
//           let process = Process()
//           let pipe = Pipe()
...
//           ]
//
//           print("Running Python script with arguments: \(process.arguments ?? [])")  // Debug: Python script and arguments
//
//           try process.run()
//           print("Python process started successfully.")  // Debug: Process started
//
//           let data = pipe.fileHandleForReading.readDataToEndOfFile()
//           if let rawOutput = String(data: data, encoding: .utf8) {
//               print("Raw output from Python process: \(rawOutput)")  // Debug: Raw output from script
//
...
//               }) {
//                   print("Found valid JSON output.")  // Debug: Found valid JSON
//                   print("Raw output from Python process: \(rawOutput)")  // Debug: Log Python output
//
//                   // Try to decode the JSON and log the decoded results
//                   if let jsonData = jsonLine.data(using: .utf8) {
//                       do {
//                           let response = try JSONDecoder().decode(SearchResponse.self, from: jsonData)
//                           print("Successfully decoded search results: \(response.results)")  // Debug: Log decoded results
//                           DispatchQueue.main.async {
...
//                           print("Error decoding JSON: \(error.localizedDescription)")  // Debug: JSON decoding error
//                       }
//                   }
//else {
//                       updateState(error: "Invalid JSON format")
//                       print("Error: Invalid JSON format")  // Debug: Invalid JSON format
//                   }
//               } else {
//                   updateState(error: "No valid JSON response found")
//                   print("Error: No valid JSON response found")  // Debug: No valid JSON response
//               }
//           } else {
//               updateState(error: "No response from search")
//               print("Error: No response from search process.")  // Debug: No response
//           }
//       } catch {
//           updateState(error: "Error during search process: \(error.localizedDescription)")
//           print("Error during search process: \(error.localizedDescription)")  // Debug: Error during search process
//       }
//   }
//
...
//           self.isSearching = false
//           self.currentTask = nil
//           print("Search process ended. Error: \(error ?? "None")")  // Debug: End of search
//       }
//   }
//}
//
